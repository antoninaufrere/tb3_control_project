...
class Tb3Controller(Node):
    def __init__(self, traj_type: str):
        ...
        self.get_logger().info(f"Using trajectory : {self.traj_type}")
        ...
        self.check_timer = self.create_timer(2.0, self.check_odom_status)

    def check_odom_status(self):
        if not self.odom_ready:
            self.get_logger().warn("Waiting Odom (Is Gazebo started ?)")
        else:
    ...
    def odom_callback(self, msg: Odometry):
        ...
        if not self.odom_ready:
            self.get_logger().info(f"Odom received ! Pose init: x={x:.2f}, y={y:.2f}")
    ...
    def control_loop(self):
        ...
        # Disturbances
        d_v = 0.02 * math.sin(0.5 * t) + 0.01 * (2.0 * random.random() - 1.)
        d_w = 0.10 * math.sin(0.7 * t) + 0.05 * (2.0 * random.random() - 1.)
    ...
    def ref_xy(self, t: float):
        if self.traj_type == 'line':
            ...
            Ax, Ay, w0 = 0.4, 0.2, 0.2
            xr = Ax * math.sin(w0 * t)
            yr = Ay * math.sin(2 * w0 * t)
            dxr = Ax * w0 * math.cos(w0 * t)
            dyr = 2 * Ay * w0 * math.cos(2 * w0 * t)
            xddr = -Ax * w0**2 * math.sin(w0 * t)
            yddr = -4 * Ay * w0**2 * math.sin(2 * w0 * t)
        else:
    ...
    def save_log(self, filename: str):
        if not self.log:
            self.get_logger().warn("Empty logs")
            return
        ...
        try:
            ...
            self.get_logger().info(f"Saving logs in : {path}")
